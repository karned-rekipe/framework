...
def _resolve_key_path(rel: Path) -> list[str]:
    ...
    if len(parts) == 3 and parts[0] == "adapters" and parts[1] == "output":
        return ["adapters", parts[2]]
    if len(parts) == 3 and parts[0] == "adapters" and parts[1] == "input":
        return [_INPUT_ALIAS.get(parts[2], parts[2])]
    if len(parts) == 2 and parts[0] == "adapters" and parts[1] == "adapters":
        return ["adapters"]
    if len(parts) == 1:
        return [parts[0]]
    return []
